...
markEmphases(const TranslationTableHeader *table, const InString *input,
	...
	for (i = 0; i < input->length; i++) {
		...
		if (checkAttr(input->chars[i], CTC_UpperCase, 0, table)) {
			 ...
			 * the next uppercase */
			/* characters that cancel caps are handled later in resolveEmphasisResets */
			if (checkAttr(input->chars[i], CTC_Letter, 0, table) &&
					checkAttr(input->chars[i], CTC_LowerCase, 0, table)) {
				emphasisBuffer[caps_start].begin |= capsEmphClass;
				if (caps_cnt) {
					/* a passage can not end on a word without letters, so if caps did not
					 * start inside the current word, end it after the last word that
					 * contained a uppercase, and start over from the beginning of the
					 * current word */
